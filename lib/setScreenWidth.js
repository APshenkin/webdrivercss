...
module.exports = function(done) {
    ...
    async.waterfall([
         ...
         */
        function(cb) {
            if (!that.self.defaultScreenDimension && that.screenWidth && that.screenWidth.length && !that.self.isMobileApp) {
        ...
        function(cb) {
            ...
            that.instance.call(function() {
                ...
                if (!that.self.isMobileApp) {
                    that.instance.windowHandleSize(that.newScreenSize.width,that.newScreenSize.height)
                        .pause(that.viewportChangePause)
                        .call(cb);
                } else {
                    that.instance.call(cb);
                }
